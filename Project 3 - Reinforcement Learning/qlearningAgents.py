...
class QLearningAgent(ReinforcementAgent):
    ...
    def __init__(self, **args):
		...
		self.values = util.Counter() # Used to store the Q-values
    ...
    def update(self, state, action, nextState, reward):
		...
		newValue = (1 - self.alpha) * self.getQValue(state, action) + self.alpha * (reward + self.discount * self.getValue(nextState))
		# the getValue function corresponds to taking the max of the Q-values over the next actions at the next state -> max(a')[Q(s', a')]

		self.values[state, action] = newValue # Update the value with the calculated value from a running average
